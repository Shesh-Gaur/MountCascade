...
void Camera::Perspective(float fovy, float aspect, float zNear, float zFar)
{
	...
	m_projection = mat4(
		vec4(xs,  0.f, 0.f, 0.f), //3 changes x translations
		vec4(0.f, ys,  -10.f, 0.f), //3 changes y translation
		vec4(0.f, 0.f, B,  0.f), //4 changes focal point z
		vec4(0.f, 0.f, C,   10.f)//4 changes focal point y
	);
...
void Camera::SetPosition(vec3 newPosition)
{
	...
	else
	{
		...
		//Calculating Orthographic Projection Matrix Manually
		float ys = 1.0f / tanf(ToRadians(m_fov.y) * 0.5f);
		float xs = ys / m_aspectRatio;
		float nmf = m_near - m_far;
		float B = (m_near + m_far) / nmf;
